...
public class PagerTag
	...
	public Object processEndLayoutEvent(EndLayoutEvent in_event) throws JspException {
		...
		if (lc_source instanceof PagerContainer) {
				...
				url = SortUtil.getURLForCollection("changer_page", (HttpServletRequest)pageContext.getRequest());				
				lc_printPager = false;							
		}
		// Print the pager
		if (lc_printPager) {
			...
			// prints the pager at the bottom
			TagUtils.write(pageContext, sb.toString());
            System.out.print("trahh="+Integer.parseInt(maxPageItems));
	...
	protected int getCurrentPage() {
		...
		String lc_possibleValue =
			(String) pageContext.getRequest().getAttribute(PAGE_NUMBER_ATTRIBUTE_KEY);
