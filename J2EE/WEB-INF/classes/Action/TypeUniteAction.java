package Action;

import java.util.ArrayList;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import org.apache.struts.action.Action;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;
import Bean.TypeUniteForm;
import Dao.Conn;
import Dao.TypeUniteDao;;

public class TypeUniteAction extends Action {
	
		 
		
		
		@Override
		public ActionForward execute(ActionMapping mapping, ActionForm form,
				HttpServletRequest request, HttpServletResponse response)
				throws Exception {
		    Conn cnx=new Conn();
			String frd = Utilitaire.connecte(request);
			if(frd.equals("ok"))
			{
			
			String action = request.getParameter("action");
			HttpSession session = request.getSession();
	
		    
		    request.setAttribute("x", null);
		    session.removeAttribute("listevide");
		    session.removeAttribute("erreur");
		    session.removeAttribute("sms");
		    TypeUniteDao ldao =new TypeUniteDao(cnx.getcnx());
		    
		    try{

	        if(action.equals("ajout"))
	        {   session.setAttribute("unite", new TypeUniteForm());
	        	boolean test=true;
	        	TypeUniteForm tfrm=(TypeUniteForm) form;
	            if(tfrm.getId_unite()==null||tfrm.getId_unite().equals("")||tfrm.getId_unite().indexOf('&')!=-1)
	            session.setAttribute("erreur", "er_invalide");
	            else{
	        	test=ldao.add(tfrm);
	            
	        	if(test==false)
	        	{session.setAttribute("erreur", "er_existant");
		    	session.setAttribute("unite", new TypeUniteForm("",tfrm.getLibelle_unite(),tfrm.getDecimale()));
	        	}
	        	else
	        	{session.setAttribute("sms", "ajout_ok");}}
	        	
	        	ArrayList<TypeUniteForm> liste=new ArrayList<TypeUniteForm>();
		    	liste=ldao.all();
		    	session.setAttribute("liste_unite", liste);
		      	if(liste.size()==0)
			    	session.setAttribute("listevide", "rien");
	
	        	
	        }
	        if(action.equals("edit"))
	        {
	        	TypeUniteForm tfrm=(TypeUniteForm) form;
	        	TypeUniteForm aux=ldao.recup(tfrm.getId_unite());
	        	if(aux==null)
	  		  {session.setAttribute("erreur", "er_modif");
	  		   session.setAttribute("unite", new TypeUniteForm());
	  		  }
	        	else{boolean modif=ldao.update(tfrm);
	        	        if(modif)
	        	        {session.setAttribute("sms", "modif_ok");}
	        	        else session.setAttribute("erreur", "er_modif1");
	        	     }
	        	ArrayList<TypeUniteForm> liste=new ArrayList<TypeUniteForm>();
		    	liste=ldao.all();
		    	session.setAttribute("liste_unite", liste);
		    	session.setAttribute("unite",new TypeUniteForm());
		    	session.removeAttribute("x");
	  		  

				

	        }
	        
	        
	        
	        
	        if(action.equals("modif"))
	        {
	      String id=request.getParameter("id");
	      TypeUniteForm aux=ldao.recup(id);
		  if(aux==null)
		  {session.setAttribute("erreur", "er_modif");
		  session.setAttribute("unite", new TypeUniteForm());
		  session.removeAttribute("x");
		  }
		  else{
		  TypeUniteForm x= ldao.recup(id);
	      session.setAttribute("unite",  x);
	      session.setAttribute("x", "");
		  }


	        }  
		    
			if(action.equals("supp"))
			{   
				String id=request.getParameter("id");
				TypeUniteForm aux=ldao.recup(id);
				if(aux==null)
				session.setAttribute("erreur", "er_suppinex");
				else{
				boolean test=ldao.delete(id);
				if(!test)
				session.setAttribute("erreur", "er_suppcascade");
				else{session.setAttribute("sms", "supp_ok");}
				}
		    	ArrayList<TypeUniteForm> liste=new ArrayList<TypeUniteForm>();
		    	liste=ldao.all();
		    	if(liste.size()==0)
		    	session.setAttribute("listevide", "rien");
		    	session.setAttribute("liste_unite", liste);
		    	session.setAttribute("unite", new TypeUniteForm());
		    	
		    	session.removeAttribute("x");
			
			}
			
			   if(action.equals("all"))
			    {
				
			    	ArrayList<TypeUniteForm> liste=new ArrayList<TypeUniteForm>();
			    	liste=ldao.all();
			    	
			    	if(liste.size()==0)
			    	session.setAttribute("listevide", "rien");
			    	session.setAttribute("liste_unite", liste);
			    	session.setAttribute("unite", new TypeUniteForm());
			    	session.removeAttribute("x");
			   
			    }
			   
		
			   
		}catch(Exception e ){System.out.println(e);}		   
			    } cnx.closecnx();
			return mapping.findForward(frd);
		}
		
		

	
	}




