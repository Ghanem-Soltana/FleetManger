...
<head>
<title><bean:message key="ordre_mission.type"/></title>

	<link rel="stylesheet" type="text/css" href="css/accomp/jquery.asmselect.css" />
	<link rel="stylesheet" type="text/css" href="css/accomp/example.css" />
...
<jsp:include flush="true" page="Menu2.jsp"></jsp:include>
	...
	<script type="text/javascript" src="js/accomp/jquery.asmselect.js"></script>
...
<br/>
<b><bean:message key="ordre_mission.type"/></b>
<br/>
<br/>

<%String titre="ajout.ordre_mission"; %>
<logic:present name="x">
<%titre="edit.ordre_mission"; %>
</logic:present>

<html:hidden name="ordre_mission" property="id_ordre_mission" styleId="id1" ></html:hidden>
<html:hidden name="ordre_mission" property="id_annee" styleId="annee1" ></html:hidden>
<html:hidden name="ordre_mission" property="marchandise_transporte" styleId="length" ></html:hidden>
<html:hidden name="ordre_mission" property="objectif" styleId="length1" ></html:hidden>

<html:form action="ordre_mission.do?action=ajout" styleId="form">
	<lay:panel align="center" key="<%=titre %>" styleClass="FORM">

		<logic:present name="x">

                    <tr>
                    <td align="left"><html:button property="" styleId="b1" indexed="b3" onclick="if(document.getElementById('id1').value!='')document.location.href='ordre_mission.do?action=pred&id='+document.getElementById('id1').value+'&annee='+document.getElementById('annee1').value ;else vide() " ><bean:message key="pred"/>  </html:button></td>
                 ...
                 <b>  <bean:message key="articles_pos"/> <%=pos %>/<%=total %></b>
                    ...
                    <td align="right"><html:button property="" styleId="b1" indexed="b4" onclick="if(document.getElementById('id1').value!='')document.location.href='ordre_mission.do?action=sui&id='+document.getElementById('id1').value+'&annee='+document.getElementById('annee1').value ;else vide()  " ><bean:message key="sui"/> </html:button></td>
			...
			<tr>
				...
				<td align="left"><html:text name="ordre_mission" property="id_ordre_mission" size="10" maxlength="6" styleId="id" onkeypress="return numeric(event,'id')"></html:text></td>
				</logic:notPresent>
				
				 <logic:present name="x">
				<td align="left"><html:text readonly="true" name="ordre_mission" property="id_ordre_mission" size="10" maxlength="6" styleId="id" ></html:text></td>
				</logic:present>
				...
				<td align="left">
	                <html:select property="id_annee" name="ordre_mission" styleId="select_annee">
...
%><html:text styleId="date" property="date_ordre_mission"  name="ordre_mission" size="10" maxlength="10" value="<%=date%>" onkeypress="return block()"></html:text>
				</logic:notPresent  >
				...
				<td align="left">
				<html:text styleId="station" property="libelle_station"  name="ordre_mission" size="30" maxlength="30" onkeypress="return block()"></html:text>
			...
			<tr>
				...
				<td align="left">
				     <html:select property="id_vehicule" name="ordre_mission" styleId="select_vehicule" onchange="metre_a_jour_station()">
				...
				<td align="left">
				   <html:select property="id_agent" name="ordre_mission" styleId="select_agent">
				...
				<tr>
			<td align="left"> <bean:message key="ordre_mission.obj"/></td>
			<td align="left" colspan="2">
			
			<html:textarea name="ordre_mission" property="objectif"  styleId="objectifsss"  onkeydown="comp2(event)" cols="40" rows="2"></html:textarea>
		...
		<tr>
		<td></td>
			<td align="left"><input type="text" id="cpt1"  maxlength="5" style="border-width:0px;height:20px;width:40px;background: transparent;" value="200" onkeypress="return block()"></input><bean:message key="caractere_restant"/></td>
			<td></td>
			<td></td>
			...
			<html:text name="ordre_mission" property="depart"  styleId="depart" size="60" maxlength="60"></html:text>
			...
			<html:text name="ordre_mission" property="destination"  styleId="ddest" size="60" maxlength="60"></html:text>
	...
	<tr>
			...
			<html:textarea name="ordre_mission" property="marchandise_transporte"  styleId="marchandise" onkeydown="comp(event)" cols="40" rows="2"></html:textarea>
       ...
       <td align="left" valign="top" colspan="3">
       	<select  id="agents" multiple="multiple" name="agents[]" title="Choisir accompagnant(s)" >
                      
                      
                        					<logic:iterate id="j"  name="liste_agent_1">
										    <bean:define id="temp2"  property="id_agent" name="j"></bean:define>
											<bean:define id="temp3"  property="libelle_agent" name="j"></bean:define>	   
                                            <option value="<%=temp2%>" ><%=temp3 %> </option>      
                                             </logic:iterate>
	
	
											<logic:iterate id="j"  name="liste_agent_2">
										    <bean:define id="temp2"  property="id_agent" name="j"></bean:define>
											<bean:define id="temp3"  property="libelle_agent" name="j"></bean:define>	   
                                            <option selected="selected" value="<%=temp2%>" ><%=temp3 %> </option>      
                                             </logic:iterate>
...
<logic:notPresent name="x"> 
			...
			<%String etat="";
			try{
			Bean.OrdreMissionForm ordre=(Bean.OrdreMissionForm )request.getSession().getAttribute("ordre_mission");
			etat=ordre.getValide();
			...
			<tr>
				<td align="center"><html:button styleId="b1" property="" indexed="b2" onclick="if(document.getElementById('id').value!=''){window.open('ordre_mission.do?action=print&id_ordre_mission='+document.getElementById('id').value+'&id_exercice='+document.getElementById('anneee').value) ;}else vide(); " >   <bean:message key="imprime_centre"/> </html:button></td>
				...
				<td align="center"><html:button styleId="b1" property="" indexed="b1" onclick="document.getElementById('form').action='ordre_mission.do?action=edit&id='+document.getElementById('id').value+'annee='+document.getElementById('anneee').value+'&i='+document.URL.split('i=')[1] ;if(document.getElementById('id').value==''){vide()}else{ if(nonvide())document.getElementById('form').submit();}" >  <bean:message key="mode_ajout_modif"/></html:button> </td>
				<td align="center"><html:button styleId="b1" property="" onclick="document.location.href='ordre_mission.do?action=all'"  > <bean:message key="mode_ajout_articles"/></html:button> </td>
			</tr>
			</logic:present>
...
document.getElementById('cpt1').value=document.getElementById('cpt1').value-document.getElementById('length1').value.length
document.getElementById('cpt1').style.color='white'
document.getElementById('cpt1').style.fontWeight = 'bold'
...
<lay:popup key="java_confirm_titre" styleClass="FORM3" styleId="popup">
	...
	<table border="0" ><tr><td><lay:message key="java_supp_confirm_ordre_mission"/></td>
...
if(id=='')
{test=false
$.jGrowl(image+'<bean:message key="ordre_mission.java.id"/>'+fin);
rouge('id')
...
if(vehicule==0)
{test=false
$.jGrowl(image+'<bean:message key="ordre_mission.java.vehi"/>'+fin);
...
if(vehicule==0)
{test=false
$.jGrowl(image+'<bean:message key="ordre_mission.java.agent"/>'+fin);
rouge('select_agent')
}
return test
...
function supp2()
{window.location.href='ordre_mission.do?action=supp&id='+document.getElementById('cd').value+'&annee='+document.getElementById('cd1').value}
...
{image='<table><tr><td><img border="0" src="images/attention.png"></td><td align="center">'
fin='</td></tr></table></center>'
$.jGrowl(image+'<bean:message key="ordre_mission.java.vide"/>'+fin);
}
...
{window.location.href='ordre_mission.do?action=modif&id='+cd+'&annee='+cd1}
...
document.getElementById('marchandise').value=document.getElementById('marchandise').value.substring(0,199); 
 ...
 function comp2(evt)
 {var keyCode = evt.which ? evt.which : evt.keyCode;
 textbox = document.getElementById( 'cpt1');
 contenu_txt2 = document.getElementById( 'objectifsss').value;


longueur= maximum -contenu_txt2.length;
 
 if(longueur<=0)
{longueur=0;
document.getElementById('objectifsss').value=document.getElementById('objectifsss').value.substring(0,199); 
}


	document.getElementById( 'cpt1').value=longueur;
	if(	document.getElementById('cpt1').value=='201')
	document.getElementById( 'cpt1').value='200';
